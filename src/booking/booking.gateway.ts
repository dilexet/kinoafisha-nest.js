...
export class BookingGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleBlockSeat(client: Socket, payload: Payload): Promise<void> {
    const result = await this.bookingService.blockSessionSeat(payload.sessionSeatId);
    this.server.emit('booking-receive:block', { ...payload, sessionSeatId: result });
  }

  @SubscribeMessage('booking:unlock')
  async handleUnlockSeat(client: Socket, payload: Payload): Promise<void> {
    const result = await this.bookingService.unlockSessionSeat(payload.sessionSeatId);
    this.server.emit('booking-receive:unlock', { ...payload, sessionSeatId: result });
  }

  @SubscribeMessage('booking:unlockAll')
  async handleUnlockAllSeats(client: Socket, payload: PayloadArray): Promise<void> {
    const result = await this.bookingService.unlockSessionSeats(payload.sessionSeatIds);
    this.server.emit('booking-receive:unlockAll', { ...payload, sessionSeatIds: result });
  }
