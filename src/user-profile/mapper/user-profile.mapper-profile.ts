  ...
  override get profile(): MappingProfile {
    return (mapper) => {
      createMap(
        ...
        BookedSeatViewDto,
        forMember(
          (dest) => dest.sessionSeatId,
          mapFrom((source) => source.id),
        ),
        forMember(
          (dest) => dest.numberSeat,
      ...
      createMap(
        ...
        forMember(
          (dest) => dest.startDate,
          mapFrom((source) =>
            convertDate(source?.sessionSeats[0]?.session?.startDate),
          ),
        ),
        forMember(
          (dest) => dest.endDate,
          mapFrom((source) =>
            convertDate(source?.sessionSeats[0]?.session?.endDate),
          ),
        ),
        forMember(
          (dest) => dest.movieName,
        ...
        forMember(
          (dest) => dest.moviePosterURL,
          mapFrom(
            (source) => source?.sessionSeats[0]?.session?.movie?.posterURL,
          ),
        ),
        forMember(
          (dest) => dest.cinemaName,
          mapFrom(
            (source) => source?.sessionSeats[0]?.session?.hall?.cinema?.name,
          ),
        ),
        forMember(
          (dest) => dest.hallName,
      ...
      createMap(
        ...
        forMember(
          (dest) => dest.isActivated,
          mapFrom((source) => source.isActivated),
        ),
        forMember(
          (dest) => dest.isRegisteredLocal,
