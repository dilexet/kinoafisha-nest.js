...
export class MovieFilterController {
  ...
  async findAll(
    @Res() res: Response,
    @Query() movieFilterQuery: MovieFilterQueryDto,
  ) {
    const result = await this.movieFilterService.findMoviesAsync(
      movieFilterQuery,
    );
  ...
  async findAllByFilters(
    @Res() res: Response,
    @Query() movieFilterQuery: MovieFilterAfishaQueryDto,
  ) {
    const result = await this.movieFilterService.findMoviesByFilterAsync(
      movieFilterQuery,
    );
    return res.status(HttpStatus.OK).json(result);
  }
