...
export class MovieFilterService {
  ...
  async findMoviesAsync(
    movieFilterQuery: MovieFilterQueryDto,
  ): Promise<MovieViewDto[]> {
    if (JSON.parse(String(movieFilterQuery?.onlyFuture ?? 'false')) === true) {
      ...
      return this.mapper.mapArray(
        movies.filter((x) => x?.sessions?.length <= 0),
        Movie,
        MovieViewDto,
      );
    }
  ...
  async findMoviesByFilterAsync(
    movieFilterQuery: MovieFilterAfishaQueryDto,
  ): Promise<MovieViewDto[]> {
    ...
    movieFilterQuery.limit =
      +movieFilterQuery?.limit <= 0 ? 1 : +movieFilterQuery?.limit;
    movieFilterQuery.page =
      +movieFilterQuery?.page <= 0 ? 1 : +movieFilterQuery?.page;
  ...
  async findMovieWithSessionsAsync(id: string): Promise<MovieDetailsViewDto> {
    ...
    const movieDetails = this.mapper.map(
      movieExist,
      Movie,
      MovieDetailsViewDto,
    );
    ...
    for (const session of movie?.sessions) {
      const cinemaExist = cinemaSessions.find(
        (x) => x?.cinema?.id == session.hall.cinema.id,
      );
      if (!cinemaExist) {
        const cinemaSession = new CinemaSessionsViewDto();
        cinemaSession.cinema = this.mapper.map(
          session.hall.cinema,
          Cinema,
          CinemaViewDto,
        );
        const sessionsCinema = movie?.sessions.filter(
          (x) => x.hall.cinema.id == cinemaSession.cinema.id,
        );
        cinemaSession.sessions = this.mapper.mapArray(
          sessionsCinema,
          Session,
          SessionViewDto,
        );
